...
class PrliLink
{
    ...
    function &get_or_create_Smart_link_for_target_url( $target_url, $group=0 )
    {
      $Smart_link_id = $this->find_first_target_url( $target_url );
      $Smart_link = $this->getOne($Smart_link_id);

      if(empty($Smart_link) or !$Smart_link)
      {
        $pl_insert_id = prli_create_Smart_link( $target_url, '', '', '', $group );
        $Smart_link = $this->getOne($pl_insert_id);
      }
      else
        prli_update_Smart_link( $Smart_link->id, '', '', '', '', $group );

      if( !isset($Smart_link) or
          empty($Smart_link) or
          !$Smart_link )
        return false;
      else
    ...
    function &is_Smart_link($url, $check_domain=true)
    {
      global $prli_blogurl;
      ...
      if( !$check_domain or preg_match( '#^' . preg_quote( $prli_blogurl ) . '#', $url ) )
      {
        ...
        if(preg_match($match_str, $uri, $match_val))
        {
          ...
          $params = $match_val[3];
          if( $Smart_link_found =& $this->is_Smart_link_slug( $match_val[2] ) )
            return compact('Smart_link_found','Smart_link_params');
    ...
    function &is_Smart_link_slug($slug)
    {
    ...
    function get_Smart_link_url($slug)
    {
      global $prli_blogurl;
    ...
    function validate( $values )
    {
      ...
      if( $values['slug'] == null or $values['slug'] == '' )
        $errors[] = "Smart Link can't be blank";

      if( $values['url'] == $prli_blogurl.PrliUtils::get_permalink_pre_slug_uri().$values['slug'] )
        $errors[] = "Target URL must be different than the Smart Link";
      ...
      if( preg_match('/^[\?\&\#]+$/', $values['slug'] ) )
        $errors[] = "Smart Link slugs must not contain question marks, ampersands or number signs.";

      if( preg_match('#/$#', $values['slug']) )
        $errors[] = "Smart Link slugs must not end with a slash (\"/\")";

      if( !$prli_utils->slugIsAvailable($values['slug'],$values['id']) )
        $errors[] = "This Smart Link Slug is already taken. Check to make sure it isn't being used by another Smart Link, post, page, category or tag slug. If none of these are true then check to see that this slug isn't the name of a file in the root folder of your wordpress install.";
