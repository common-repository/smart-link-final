...
class PrliUtils
{
  ...
  */
  ...
  function track_link($slug,$values)
  {
    ...
    $Smart_link = $wpdb->get_row($query);
    $Smart_link_target = apply_filters('prli_target_url',array('url' => $Smart_link->url, 'link_id' => $Smart_link->id));
    $Smart_link_url = $Smart_link_target['url'];
    
    if(isset($Smart_link->track_me) and $Smart_link->track_me)
    {
      $first_click = 0;
      ...
      $cookie_name = 'prli_click_' . $Smart_link->id;
      ...
      $insert = $wpdb->prepare($insert_str, $Smart_link->id,
                                            $visitor_uid,
                                            $click_ip,
                                            $click_user_agent,
      ...
      do_action('prli_record_click',array('link_id' => $Smart_link->id, 'click_id' => $wpdb->insert_id, 'url' => $Smart_link_url));
    }
  
    // Reformat Parameters
    $param_string = '';
      
    if(isset($Smart_link->param_forwarding) and ($Smart_link->param_forwarding == 'custom' OR $Smart_link->param_forwarding == 'on') and isset($values) and count($values) >= 1)
    {
      $first_param = true;
      foreach($values as $key => $value)
      {
        if($first_param)
        {
          $param_string = (preg_match("#\?#", $Smart_link_url)?"&":"?");
          $first_param = false;
        }
        else
          $param_string .= "&";
    ...
    if(isset($Smart_link->nofollow) and $Smart_link->nofollow)
      header("X-Robots-Tag: noindex, nofollow", true);

    switch($Smart_link->redirect_type)
    {
      case '301':
        header("HTTP/1.1 301 Moved Permanently");
        header('Location: '.$Smart_link_url.$param_string);
        break;
      default:
        if( $Smart_link->redirect_type == '307' or
            !$prli_update->pro_is_installed_and_authorized() )
        {
          ...
          do_action('prli_issue_cloaked_redirect', $Smart_link->redirect_type, $Smart_link_url, $param_string);
    }
  }
  ...
  // be careful with this one -- I use it to forceably reinstall Smart Link pro
  ...
  function migrate_before_db_upgrade()
  {
    ...
    if($db_version and $db_version < 1)
    {
      ...
      foreach($links as $link)
      {
        ...
        else if(isset($link->use_Smartbar) and $link->use_Smartbar)
        {
          $query = $wpdb->prepare($query_str, 'Smartbar', $link->id);
          $wpdb->query($query);
        }
      ...
      $query = "ALTER TABLE {$prli_link->table_name} DROP COLUMN track_as_img, DROP COLUMN use_Smartbar, DROP COLUMN use_ultra_cloak, DROP COLUMN gorder";
      $wpdb->query($query);
    }
    ...
    if($db_version and $db_version < 2)
    {
      ...
      unset($prli_options->Smartbar_image_url);
      unset($prli_options->Smartbar_background_image_url);
      unset($prli_options->Smartbar_color);
      unset($prli_options->Smartbar_text_color);
      unset($prli_options->Smartbar_link_color);
      unset($prli_options->Smartbar_hover_color);
      unset($prli_options->Smartbar_visited_color);
      unset($prli_options->Smartbar_title_limit);
      unset($prli_options->Smartbar_desc_limit);
      unset($prli_options->Smartbar_link_limit);
    ...
    {
      // Install / Upgrade Smart Link Pro
    ...
    if($db_version and $db_version < 11)
    {
      /* Too much to handle on larger tweet databases -- this code will still be accessible by going to the following url:
         {$prli_blogurl}/wp-admin/admin.php?page=Smart-link/pro/prlipro-options.php&action=trim_dup_tweets
  ...
  function migrate_after_db_upgrade()
  {
    ...
    {
      // Migrate Smart-link-posted-to-twitter
      $query = "SELECT * FROM {$wpdb->prefix}postmeta WHERE meta_key=%s";
      $query = $wpdb->prepare($query,'Smart-link-posted-to-twitter');
      ...
      foreach($posts_posted as $postmeta)
      {
        ...
        {
          $link_id = PrliUtils::get_prli_post_meta($postmeta->post_id,'Smart-link',true);
          $prli_link_meta->update_link_meta($link_id,'Smart-link-posted-to-twitter','1');
        }
      }

      // Cleanup
      $query = "DELETE FROM {$wpdb->prefix}postmeta WHERE meta_key=%s OR meta_key=%s OR meta_key=%s OR meta_key=%s";
      $query = $wpdb->prepare($query,'Smart-link-posted-to-twitter','Smart-link-tweet-count','Smart-link-tweet-last-update','prli-keyword-replacement-count');
  ...
  /** Gets rid of any Smart Link postmetas created without a post_id **/
  function clear_unknown_post_metas()
  {
    global $wpdb;

    $query = "SELECT count(*) FROM {$wpdb->postmeta} WHERE ( meta_key LIKE 'prli%' OR meta_key LIKE 'Smart-link%' OR meta_key LIKE '_prli%' OR meta_key LIKE '_Smart-link%' ) AND post_id=0";
    $count = $wpdb->get_var($query);

    if($count)
    {
      $query = "DELETE FROM {$wpdb->postmeta} WHERE ( meta_key LIKE 'prli%' OR meta_key LIKE 'Smart-link%' OR meta_key LIKE '_prli%' OR meta_key LIKE '_Smart-link%' ) AND post_id=0";
      $wpdb->query($query);
    }
  }
}
