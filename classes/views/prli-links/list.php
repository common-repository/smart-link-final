...
?>
  <h2><img src="<?php echo PRLI_URL.'/images/Smart-link-med.png'; ?>"/>&nbsp;Smart Link: Links</h2>
<h1 style="text-align: left">For Instructions Visit:<a href="http://111waystomakemoney.com/smart-link/" target="_blank">Plugin Page</a></h1>
  ...
  if(empty($params['group']))
  {
    ...
    {
    ?>
      <div class="error" style="padding-top: 5px; padding-bottom: 5px;"><strong>WordPress Must be Configured:</strong> Smart Link won't work until you select a Permalink Structure other than "Default" ... <a href="<?php echo $prli_siteurl; ?>/wp-admin/options-permalink.php">Permalink Settings</a></div>
    <?php
    }
  ?>
  ...
  <div id="button_bar">
    <p><a href="?page=<?php echo PRLI_PLUGIN_NAME; ?>/prli-add-link.php"><img src="<?php echo PRLI_URL.'/images/Smart-link-add.png'; ?>"/> Add a Smart Link</a>
    &nbsp;|&nbsp;<a href="?page=<?php echo PRLI_PLUGIN_NAME; ?>/prli-options.php">Options</a>
    &nbsp;|&nbsp;<a href="http://blairwilliams.com/plintro">Watch Smart Link Intro Video</a>
...
<table class="widefat post fixed" cellspacing="0">
  ...
  else
  {
    ...
    foreach($links as $link)
    {
      ...
      $Smart_link_url = "{$prli_blogurl}{$struct}{$link->slug}";
      ?>
      <tr style="min-height: 75px; height: 75px;">
        <td class="edit_link">

        <?php do_action('prli_list_icon',$link->id); ?>
        <?php if( $link->redirect_type == 'Smartbar' ) { ?>
            <img src="<?php echo PRLI_URL.'/images/Smart-link-small.png'; ?>" title="Using SmartBar" width="13px" height="13px" />
        <?php }
        ...
        <?php }
          ...
          <span title="Temporary Redirection (307)" style="font-size: 15px; line-height: 15px; padding: 0px; margin: 0px; color: green;"><strong>T</strong></span>&nbsp;
        <?php }
        else if( $link->redirect_type == '301' ) { ?>
          <span title="Permanent Redirection (301)" style="font-size: 15px; line-height: 15px; padding: 0px; margin: 0px; color: green;"><strong>P</strong></span>&nbsp;
        <?php } ?>
        ...
        ?>
          ...
          <a href="<?php echo $Smart_link_url; ?>" target="_blank" title="Visit Smart Link: <?php echo $Smart_link_url; ?> in a New Window"><img src="<?php echo PRLI_URL.'/images/url_icon.gif'; ?>" width="13px" height="13px" name="Visit" alt="Visit"/></a>&nbsp;
        <?php
        }
        ?>
        <a class="slug_name" href="?page=<?php echo PRLI_PLUGIN_NAME; ?>/prli-links.php&action=edit&id=<?php echo $link->id; ?>" title="Edit <?php echo stripslashes($link->name); ?>"><?php echo stripslashes($link->name); ?></a>
          ...
          <div class="link_actions">
            ...
            <a href="?page=<?php echo PRLI_PLUGIN_NAME; ?>/prli-links.php&action=destroy&id=<?php echo $link->id; ?>"  onclick="return confirm('Are you sure you want to delete your <?php echo $link->name; ?> Smart Link? This will delete the Smart Link and all of the statistical data about it in your database.');" title="Delete <?php echo $link->slug; ?>">Delete</a>
            <?php if( $link->track_me ) { ?>
            |&nbsp;<a href="?page=<?php echo PRLI_PLUGIN_NAME; ?>/prli-links.php&action=reset&id=<?php echo $link->id; ?>"  onclick="return confirm('Are you sure you want to reset your <?php echo $link->name; ?> Smart Link? This will delete all of the statistical data about this Smart Link in your database.');" title="Reset <?php echo $link->name; ?>">Reset</a>&nbsp;|
            ...
            {
            ?>
            |&nbsp;<a href="http://twitter.com/home?status=<?php echo $Smart_link_url; ?>" target="_blank" title="Post <?php echo $Smart_link_url; ?> to Twitter">Tweet</a>&nbsp;|
            <a href="mailto:?subject=Smart Link&body=<?php echo $Smart_link_url; ?>" target="_blank" title="Send <?php echo $Smart_link_url; ?> in an Email">Email</a>
            <?php
            }
            ?>
          </div>
        ...
        <td><input type='text' style="font-size: 10px; width: 100%;" readonly="true" onclick='this.select();' onfocus='this.select();' value='<?php echo $Smart_link_url; ?>' /><br/>
        <?php if( $link->redirect_type != 'pixel' )
        {
        ?>
