<?php
/**
 * Smart Link WordPress Plugin API
 */
...
/**
 * Create a Smart Link for a long, ugly URL.
 *
 ...
 */
function prli_create_Smart_link( $target_url,
                                  $slug = '',
                                  $name = '',
                                  $description = '',
                                  $group_id = 0,
                                  ...
                                  $param_struct = '' )
{
  ...
  global $prli_error_messages, $prli_Smart_link, $prli_Smart_slug, $prli_options;
  ...
  if( count($prli_error_messages) == 0 )
  {
    ...
    {
      $prli_error_messages[] = "An error prevented your Smart Link from being created";
      return false;
    }
  }
  else
...
function prli_update_Smart_link( $id,
                                  $target_url = '',
                                  $slug = '',
                                  $name = -1,
                                  $description = -1,
...
{
  global $wpdb, $prli_link, $prli_blogurl;
  global $prli_error_messages, $prli_Smart_link, $prli_Smart_slug;

  if(empty($id))
  {
    $prli_error_messages[] = "Smart Link ID must be set for successful update.";
    return false;
  }
...
/**
 * Gets the full Smart Link url from an id
 *
 * @return bool (false if failure) | string the Smart Link url
 */
function prli_get_Smart_link_url($id)
{
  global $prli_link,$prli_blogurl;

  if($Smart_link = $prli_link->getOne($id))
    return "{$prli_blogurl}".PrliUtils::get_permalink_pre_slug_uri()."{$Smart_link->slug}";
