...
function prli_menu()
{
  add_menu_page('Smart Link', 'Smart Link', 8, PRLI_PATH.'/prli-links.php','',PRLI_URL.'/images/Smart-link-small.png'); 
  add_submenu_page(PRLI_PATH.'/prli-links.php', 'Smart Link | Add New Link', 'Add New Link', 8, PRLI_PATH.'/prli-add-link.php');
  add_submenu_page(PRLI_PATH.'/prli-links.php', 'Smart Link | Groups', 'Groups', 8, PRLI_PATH.'/prli-groups.php');
  add_submenu_page(PRLI_PATH.'/prli-links.php', 'Smart Link | Hits', 'Hits', 8, PRLI_PATH.'/prli-clicks.php');
  add_submenu_page(PRLI_PATH.'/prli-links.php', 'Smart Link | Tools', 'Tools', 8, PRLI_PATH.'/prli-tools.php');
  add_submenu_page(PRLI_PATH.'/prli-links.php', 'Smart Link | Options', 'Options', 8, PRLI_PATH.'/prli-options.php');
  add_submenu_page(PRLI_PATH.'/prli-links.php', 'Smart Link | Smart Link Pro', 'Smart Link Pro', 8, PRLI_PATH.'/prli-pro-settings.php');

  add_action('admin_head-Smart-link/prli-clicks.php', 'prli_reports_admin_header');
  add_action('admin_head-Smart-link/prli-links.php', 'prli_links_admin_header');
  add_action('admin_head-Smart-link/prli-add-link.php', 'prli_links_admin_header');
  add_action('admin_head-Smart-link/prli-groups.php', 'prli_groups_admin_header');
  add_action('admin_head-Smart-link/prli-options.php', 'prli_options_admin_header');
}
...
function prli_redirect()
{
  ...
  if( $link_info =& $prli_link->is_Smart_link($request_uri,false) )
    prli_link_redirect_from_slug( $link_info['Smart_link_found']->slug, $link_info['Smart_link_params'] );
}
...
function prli_add_dashboard_widgets() {
  ...
  {
    wp_add_dashboard_widget('prli_dashboard_widget', 'Smart Link Quick Add', 'prli_dashboard_widget_function');   
...
/********* EXPORT Smart Link API VIA XML-RPC ***********/
function prli_export_api($api_methods)
{
  $api_methods['prli.create_Smart_link']  = 'prli_xmlrpc_create_Smart_link';
  ...
  $api_methods['prli.get_Smart_link_url'] = 'prli_xmlrpc_get_Smart_link_url';
...
function prli_install()
{
  ...
  $Smart_links_table = $wpdb->prefix . "prli_links";
  ...
  if($db_version != $old_db_version)
  {
    ...
    $sql = "CREATE TABLE " . $clicks_table . " (
              ...
              //CONSTRAINT ".$clicks_table."_ibfk_1 FOREIGN KEY (link_id) REFERENCES $Smart_links_table (id)
            ") {$charset_collate};";
    
    dbDelta($sql);
    
    /* Create/Upgrade Smart Links Table */
    $sql = "CREATE TABLE " . $Smart_links_table . " (
              id int(11) NOT NULL auto_increment,
...
register_activation_hook(PRLI_PATH."/Smart-link.php",'prli_install');
...
function prli_pro_action_needed( $plugin )
{
  ...
  if( $plugin == 'Smart-link/Smart-link.php' and
...
function prli_pro_get_started_headline()
{
      ...
      !$prli_update->pro_is_installed())
  {
    ...
    ?>
<div class="error" style="padding-top: 5px; padding-bottom: 5px;"><?php printf(__('Your Smart Link Pro installation isn\'t quite complete yet.<br/>%1$sAutomatically Upgrade to Enable Smart Link Pro%2$s', 'Smart-link'), '<a href="'.$inst_install_url.'">','</a>'); ?></div>  
    <?php 
  }
}
